...
export default function HomePage() {
  return (
    <div>
      <section className="mx-auto max-w-6xl px-4 py-20 text-center">
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          ...
          <Link
            ...
            className="rounded-md border border-white/20 px-8 py-4 text-lg font-bold text-white hover:border-brand-red hover:text-brand-red transition-colors"
          >
            Request a Mechanic
          </Link>
        </div>
      ...
      <section className="border-y border-white/10 bg-brand-charcoal py-16">
        <div className="mx-auto grid max-w-6xl gap-6 px-4 sm:grid-cols-2 lg:grid-cols-5">
          {FEATURES.map((f) => (
            <div key={f.title} className="rounded-lg border border-white/10 bg-black/40 p-5">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 text-center">
        ...
        <div className="mt-8 rounded-lg border border-brand-red/40 bg-brand-red/10 p-6">
          ...
          <p className="mt-2 text-sm text-white/60">4444 Symmes Rd, Fairfield, OH</p>
        </div>
      </section>
    </div>
  );
